...
import { ConsultingRequest, CONSULTING_STATUS_LABELS } from '../../types';
...
export function PurchaseConsultingPage() {
      ...
      <div className="min-h-screen bg-white">
        <div className="container mx-auto px-4 py-12">
          <div className="max-w-lg mx-auto text-center">
            <h1 className="text-3xl mb-4">컨설팅</h1>
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await apiPost(API_ENDPOINTS.CONSULTING, { title: title.trim(), content: content.trim() });
      setSuccess('컨설팅이 신청되었습니다.');
      setTitle('');
      setContent('');
      setView('list');
      await loadRequests();
  ...
  const handleEditStart = (req: ConsultingRequest) => {
    setEditingId(req.id);
    setTitle(req.title);
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await apiPatch(API_ENDPOINTS.CONSULTING_ITEM(editingId), { title: title.trim(), content: content.trim() });
      setSuccess('수정되었습니다.');
      setEditingId(null);
      setTitle('');
      setContent('');
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl">컨설팅</h1>
              <p className="text-muted-foreground mt-1">
                선박 / 용도 / 예산에 맞춘 최적의 장비 구성 제안
              </p>
            </div>
            {view === 'list' && (
              ...
              <form onSubmit={view === 'create' ? handleCreate : handleEdit} className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="컨설팅 요청 제목을 입력하세요 (예: 어선용 어탐기 추천 요청)"
                    maxLength={150}
                    className="mt-1"
                    disabled={submitting}
                  />
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={`컨설팅 내용을 자세히 입력해주세요.\n\n예시:\n- 선박 종류 및 톤수\n- 주요 사용 목적 및 운항 지역\n- 희망 예산\n- 현재 보유 장비 (있는 경우)`}
                    rows={8}
                    disabled={submitting}
          ...
          <div className="mt-12 p-6 bg-secondary border border-border rounded-lg">
            <h4 className="mb-3 font-semibold">컨설팅 안내</h4>
